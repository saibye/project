...
def u15m_analyzer(_stock_id, _a_date, _b_date, _c_date, _d_date, _e_date, _my_df, _used_len, _db):
    ...
    if got_A and got_B and got_C and got_D and got_E:
        ...
        # D点rpv
        D1 = 8
        D_rpv_rt = u15m_rpv(_my_df, _used_len, _d_date, D1, _db)
    ...
    if to_mail:
        subject = "u15m: %s -- %s" % (_stock_id, _e_date)
        log_info(subject)
        log_info("mail:\n%s", content1)
        mailed = 1
    else:
...
def u15m_work_one_day_stock(_stock_id, _a_date, _b_date, _c_date, _d_date, _e_date, _db):
    ...
    # 之前n1单位的交易数据
    n1 = g_detail_fetched
    detail_df = get_u15m_detail(_stock_id, _a_date, n1, _db);
    ...
    rv = u15m_analyzer(_stock_id, _a_date, _b_date, _c_date, _d_date, _e_date, my_df, used_len, _db)
    if rv == 0:
...
def u15m_rpv(_detail_df, _used_len, _till, _n, _db):
    U_sum = 0.0 # up
    U_days = 0
    D_sum = 0.0 # down
    D_days = 0
...
def u15m_dynamic_calc_tech(_df):
...
def u15m_format_ref(_stock_id, _detail_df):
    ...
    u15m_dynamic_calc_tech(_detail_df)
...
def work():
    ...
    D_date = "2017-07-10"
    E_date = "2017-07-04"
    u15m_work_one_day_stock(stock_id, A_date, B_date, C_date, D_date, E_date, db)
