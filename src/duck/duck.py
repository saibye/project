...
def duck_work_one_day_stock(_stock_id, _till,  _db):
    ...
    # 之前n1单位的交易数据
    n1 = g_detail_fetched
    detail_df = get_duck_detail(_stock_id, _till, n1, _db);
    ...
    # 格式化数据
    rv = duck_format_ref(_stock_id, detail_df)
    if rv < 0:
        log_error("error: duck_format_ref: %s", _stock_id)
        return -1
    ...
    # case1
    rv = duck_analyzer1(_stock_id, _till, my_df, used_len, _db)
    if rv == 0:
...
def get_duck_detail(_stock_id, _pub_date, _n, _db):
...
def get_duck_stock_list(_till, _db):
...
def duck_work_one_day(_till_date, _db):

    log_info("date: %s", _till_date)

    list_df = get_duck_stock_list(_till_date, _db)
    if list_df is None:
        log_error("error: get_duck_stock_list failure")
        return -1
    else:
        # log_debug("list df:\n%s", list_df)
        pass
    ...
    for row_index, row in list_df.iterrows():
        ...
        duck_work_one_day_stock(stock_id, _till_date, _db)
...
def regression(_db):
    ...
    for row_index, row in date_df.iterrows():
        ...
        duck_work_one_day(till_date, _db)
...
def work():
    ...
    if sai_is_product_mode():
        ...
        # duck_work_one_day(till_date, db)

        # 西部建设 002302
        till_date = "2017-03-20"
        stock_id  = "002302"
        duck_work_one_day_stock(stock_id, till_date, db)
