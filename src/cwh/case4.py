...
# 000540
#
def CupWithHandle_analyzer4(_stock_id, _trade_date, _my_df, _used_len, _db):
    ...
    for row_index, row in _my_df.iterrows():
        ...
        if to_get_E:
            ...
            if vr > E_VR and (rate > E_RATE or rate2 > E_RATE2):
                ...
                if E_break_days > E_DAYS:
                    log_info("nice: E点即将确认: 突破:%d, 阻塞:%s", E_break_days, E_block_date)
                else:
                    return 1
                ...
                if E_vol_break_days > E_DAYS:
                    log_info("nice: E点确认: vol-突破:%d, 阻塞:%s", E_vol_break_days, E_vol_block_date)
                    to_get_E = False
                    to_get_C = True
                    got_E = True
                else:
        ...
        if to_get_A:
            ...
            # E点突破天数
            if E_break_days >= 70 and AA_vr > 50 and AA_rt > -2:
    ...
    if to_mail:
        subject = "CwH4: %s -- %s" % (_stock_id, _trade_date)
        log_info(subject)
        log_info("mail:\n%s", content1)
