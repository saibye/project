...
def rt_dadan_one(_stock_id, _dd_date, _db):
    ...
    vol_base = 3000
    df_big = df[df.volume >= vol_base]

    # check df
    vol = 3000
    cnt = 10
    pri = 10.00
    rt_dadan_check_buy(_stock_id, df_big, _db, vol, cnt, g_has_noticed,  g_good_list, pri)

    vol = 10000
    cnt = 3
    pri = 9.00
    rt_dadan_check_buy(_stock_id, df_big, _db, vol, cnt, g_has_noticed2, g_good_list2, pri)

    vol = 100000
    cnt = 1
    pri = 8.00
    rt_dadan_check_buy(_stock_id, df_big, _db, vol, cnt, g_has_noticed3, g_good_list3, pri)
...
def rt_dadan(_stocks, _trade_date, _db):
    ...
    for row_index, row in _stocks.iterrows():
        ...
        if g_has_noticed.has_key(stock_id):
            pass
        elif g_has_noticed2.has_key(stock_id):
            pass
        elif g_has_noticed3.has_key(stock_id):
            pass
        else :
            log_debug("processing: %s", stock_id)
