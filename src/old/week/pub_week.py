...
# 收盘价
#
def week_break_days(_detail_df, _used_len, _date, _my_high, _db):
...
# 成交量
#
def week_vol_break_days(_detail_df, _used_len, _date, _my_high, _db):
...
def week_rpv(_detail_df, _used_len, _till, _n, _db):
    U_sum = 0.0 # up
    U_days = 0
    D_sum = 0.0 # down
    D_days = 0
...
def week_dynamic_calc_tech(_df):
...
def week_format_ref(_stock_id, _detail_df):
    ...
    week_dynamic_calc_tech(_detail_df)
...
#
def week_preceding_high(_detail_df, _used_len, _date, _n1, _n2, _db):
    idx = 0
    days = 0
    to_start = False
    to_count = False
...
#
def week_preceding_low(_detail_df, _used_len, _date, _n1, _n2, _db):
    idx = 0
    days = 0
    low_date = ""
    to_start = False
...
# X点往前n1单位内的最高high价
#
def week_preceding_high2(_detail_df, _used_len, _date, _n1, _db):
    idx = 0
    days = 0
    to_start = False
    to_count = False
...
def week_save(_stock_id, _pub_date, _type,  _price, _title, _message, _db):
    inst_date = get_today()
    inst_time = get_time()
...
# 收盘价
#
def week_break_preceding_days(_detail_df, _used_len, _date, _my_price, _db):
...
# X, Y点之间涨停个数
#
def week_bump_units(_detail_df, _used_len, _date1, _date2, _db):
...
# X点往前有缺口
#
def week_has_gap(_detail_df, _used_len, _date, _n, _db):
