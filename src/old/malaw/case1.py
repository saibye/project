...
# 
#
def malaw_analyzer1(_stock_id, _trade_date, _my_df, _used_len, _db):
    ...
    log_debug("A点: ma5-10-200 bond3:  %.2f%%", bond3)
    ...
    # 确认B点
    d_B, i_B, rt_B, vr_B, days_BC, d_C = malaw_break_point(_my_df, _used_len, d_A, A_DAYS1, A_DAYS2, _db)
    if d_B == "":
    ...
    # A、B区域内涨停个数
    violent_unit = malaw_bump_units(_my_df, _used_len, d_A, d_B, _db)
    ...
    rpv_B = malaw_rpv(_my_df, _used_len, d_B, B_DAYS1, _db)
    ...
    if rule1 or rule2:
        to_mail = True
        log_info("bingo: %s malaw %s", _stock_id, _trade_date)
    ...
    if to_mail:
        ...
        subject = "malaw1: %s -- %s" % (_stock_id, _trade_date)

        # malaw_save(_stock_id, _trade_date, expect_price, subject, content1, _db)
