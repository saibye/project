...
# 300520 # 2017-9-7
#
def pre_thrive_analyzer4(_stock_id, _trade_date, _my_df, _used_len, _db):
    ...
    rule_K = max(o_C, p_C) > max(o_K, p_K) and max(o_K, p_K) > max(o_B, p_B)
    if rule_C and rule_B and rule_K:
        log_info("nice: C点确认: %s, 三线跌幅: %.2f", d_C, rate_BC)
    else:
    ...
    if to_mail:
        ...
        subject = "pre_thrive4: %s -- %s" % (_stock_id, _trade_date)
