...
def malaw_work_one_day_stock(_stock_id, _till,  _db):
    ...
    # 之前n1单位的交易数据
    n1 = g_detail_fetched
    detail_df = get_malaw_detail(_stock_id, _till, n1, _db);
    ...
    # case1
    rv = malaw_analyzer1(_stock_id, _till, my_df, used_len, _db)
    if rv == 0:
...
def malaw_work_one_day(_till_date, _db):

    log_info("date: %s", _till_date)

    list_df = get_malaw_stock_list(_till_date, _db)
    if list_df is None:
        log_error("error: get_malaw_stock_list failure")
        return -1
    else:
        # log_debug("list df:\n%s", list_df)
        pass
    ...
    for row_index, row in list_df.iterrows():
        ...
        malaw_work_one_day_stock(stock_id, _till_date, _db)
...
def regression(_db):
    ...
    for row_index, row in date_df.iterrows():
        ...
        malaw_work_one_day(till_date, _db)
...
def work():
    ...
    if sai_is_product_mode():
        ...
        # 002405 四维图新
        till_date = "2017-08-14"
        stock_id  = "002405"
        malaw_work_one_day_stock(stock_id, till_date, db)

        # 002158 汉钟精机
        till_date = "2017-04-05"
        stock_id  = "002158"
        malaw_work_one_day_stock(stock_id, till_date, db)

        # 002346 柘中股份 fail, but TODO
        till_date = "2016-11-10"
        stock_id  = "002346"
        malaw_work_one_day_stock(stock_id, till_date, db)

        # 600516 方大碳素
        till_date = "2017-05-24"
        stock_id  = "600516"
        malaw_work_one_day_stock(stock_id, till_date, db)
        ...
        # 600558  大西洋
        till_date = "2017-08-23"
        stock_id  = "600558"
        malaw_work_one_day_stock(stock_id, till_date, db)

        # 002362 汉王科技
        till_date = "2017-08-17"
        stock_id  = "002362"
        malaw_work_one_day_stock(stock_id, till_date, db)

        # 000799 酒鬼酒
        till_date = "2017-07-14"
        stock_id  = "000799"
        malaw_work_one_day_stock(stock_id, till_date, db)

        # 600117 西宁特钢
        till_date = "2017-07-20"
        stock_id  = "600117"
        malaw_work_one_day_stock(stock_id, till_date, db)
