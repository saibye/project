...
def k_pro_one_to_db(_stock_id, _df, _start_date, _db):
...
def k_pro_get_max_date(_stock_id, _db):
...
def k_pro_one_stock(_stock_id, _db):
    # log_info("k_pro_one_stock begin")

    # get max-date from table, as start date
    max_date = k_pro_get_max_date(_stock_id, _db)
    ...
    try:
        pro = ts.pro_api()
        df = pro.daily(ts_code='000002.SZ', start_date='20180701', end_date='20180918')
        log_debug('df\n%s', df)
    ...
    # k_pro_one_to_db(_stock_id, df, max_date, _db)

    log_info("one_to_db costs %d us", get_micro_second() - begin)

    # log_info("function k_pro_one_stock end")

    return 



def k_pro_one_check_bad(_stock_id, _db):
    is_bad = False

    # log_info("k_pro_one_check begin")
...
def work():
    ...
    ts.set_token('f9592696577093c0cdbfa54318d8f7140f82d9af7c7dca36b8bd4843')

    stock_id = "600050"
    stock_id = "002460"
    stock_id = "000932"
    log_debug("stock: %s", stock_id)
    k_pro_one_stock(stock_id, db)
    return 0
    """
    """
    ...
    for row_index, row in stocks.iterrows():
        ...
        # check bad data
        # stock_id = "002837"
        k_pro_one_check_bad(stock_id, db)
        # break
    ...
    """
    stock_id = "700002"
    start = k_pro_get_max_date(stock_id, db);
