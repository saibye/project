...
# 603559
#
def thrive_analyzer15(_stock_id, _trade_date, _my_df, _used_len, _db):
    mailed = 0
    ...
    content1 += "%s -- %s\n" % (_stock_id, d_A)
    content1 += "SPECIAL: A点突破大量收盘！\n"
    content1 += "SPECIAL: A点放量，BC明显缩量！\n"
    content1 += "SPECIAL: 收复4线+++\n"
    ...
    if to_mail:
        subject = "thrive15(rare): %s -- %s" % (_stock_id, _trade_date)
        log_info(subject)
        log_info("mail:\n%s", content1)
