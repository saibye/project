...
# 000710
#
def thrive_analyzer12(_stock_id, _trade_date, _my_df, _used_len, _db):
    mailed = 0
    ...
    # D点往前一字板
    unit_D = thrive_preceding_one(_my_df, _used_len, d_D, D_DAYS3, _db)
    log_info("D点往前一字板的个数: %d", unit_D)

    # D点RPV
    rpv_D = thrive_rpv(_my_df, _used_len, d_D, D_DAYS3, _db)
    log_debug("rpv(D点): %.2f", rpv_D)
    if rpv_D < 3 and unit_D < 2:
    ...
    if to_mail:
        subject = "thrive12: %s -- %s" % (_stock_id, _trade_date)
        log_info(subject)
        log_info("mail:\n%s", content1)
